...
////// Emissive:
                ...
                float4 node_59 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_94, _MainTex),0.0,node_110));
                float node_154 = 0.0;
                ...
                float4 node_260 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_270, _MainTex),0.0,node_110));
                float2 node_272 = (node_86*5.0); // Penta Offset
                float2 node_268 = (node_120+node_272); // unused
                float4 node_258 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_268, _MainTex),0.0,node_110));
                float2 node_238 = (node_86*4.0); // Quad Offset
                float2 node_236 = (node_120+node_238); // plus plus plus plus
                float4 node_230 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_236, _MainTex),0.0,node_110));
                float2 node_200 = (node_86*3.0); // Tripple Ofsset
                float2 node_198 = (node_120+node_200); // minus minus
                float4 node_204 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_198, _MainTex),0.0,node_110));
                float2 node_124 = (node_86*2.0); // Double Offset
                float2 node_72 = (node_120+node_124); // plus plus
                float4 node_61 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_72, _MainTex),0.0,node_110));
                float2 node_90 = (node_120+node_86); // plus
                float4 node_63 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_90, _MainTex),0.0,node_110));
                float2 node_87 = (node_120-node_86); // minus
                float4 node_57 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_87, _MainTex),0.0,node_110));
                float2 node_92 = (node_120-node_124);
                float4 node_2 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_92, _MainTex),0.0,node_110));
                float2 node_196 = (node_120-node_200); // plus plus
                float2 node_234 = (node_120-node_238); // minus minus minus minus
                float4 node_232 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_234, _MainTex),0.0,node_110));
                float2 node_264 = (node_120-node_272); // m m m m m
                float4 node_256 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_264, _MainTex),0.0,node_110));
                float2 node_266 = (node_120-node_274); // pp
                float4 node_262 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_266, _MainTex),0.0,node_110));
                float3 emissive = lerp(node_59.rgb,lerp(((((node_260.rgb+node_258.rgb+node_230.rgb+node_204.rgb+node_61.rgb)+node_63.rgb+node_59.rgb+node_57.rgb+node_2.rgb)+tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_196, _MainTex),0.0,node_110)).rgb+node_232.rgb+node_256.rgb+node_262.rgb)/13.0),(((node_230.rgb+node_232.rgb)+((node_204.rgb+node_204.rgb)*pow(2.0,1.0))+((node_61.rgb+node_2.rgb)*pow(2.0,2.0))+((node_63.rgb+node_57.rgb)*pow(2.0,3.0))+(node_59.rgb*32.0))/64.0),0.0),saturate(node_84));
